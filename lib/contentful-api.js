...
const GLOSSARY_QUERY = `query {
  page(id: "5xKPbVVUwK9euTca9tAT70") {
    title
    content {
      json
    }
...
const ABOUT_PAGE_QUERY = `query {
  page(id: "2HxUbIqRT4MQheh7m1ZI6g") {
    title
    content {
      json
    }
...
const ACKNOWLEDGEMENTS_PAGE_QUERY = `query {
  page(id: "RgjyOCRQR6R3gJ3qKEGCe") {
    title
    content {
      json
    }
...
const METHODOLOGY_PAGE_QUERY = `query {
  page(id: "72DMGKLKj2Ig0jQMbAc1Ui") {
    title
    content {
      json
    }
...
if (!process.env.NEXT_CONTENTFUL_API_ENDPOINT)
  throw new Error("Missing NEXT_CONTENTFUL_API_ENDPOINT");
if (!process.env.CONTENTFUL_ACCESS_TOKEN)
  throw new Error("Missing CONTENTFUL_ACCESS_TOKEN");

export async function fetchGraphQL(query, variables) {
  return fetch(process.env.NEXT_CONTENTFUL_API_ENDPOINT, {
    method: "POST",
    headers: {
    ...
    .then((json) => {
      if (!json.data)
        throw new Error(
          JSON.stringify(json.errors.map((e) => e.message).join(", "), null, 2)
        );
      return json;
    });
}
...
export async function getAboutPage() {
  const res = await fetchGraphQL(ABOUT_PAGE_QUERY);
  return res.data.page;
}

export async function getAcknowledgmentsPage() {
  const res = await fetchGraphQL(ACKNOWLEDGEMENTS_PAGE_QUERY);
  return res.data.page;
}

export async function getMethodologyPage() {
  const res = await fetchGraphQL(METHODOLOGY_PAGE_QUERY);
  return res.data.page;
}

export async function getGlossary() {
  const res = await fetchGraphQL(GLOSSARY_QUERY);
  return res.data.page;
}
...
export async function getScores() {
  const res = await fetchGraphQL(SCORES_QUERY);
  return res?.data?.scoreCollection?.items ?? [];
}
