...
const documentOptions = {
  renderNode: {
    [BLOCKS.HEADING_3]: (_, children) => <strong className='state-to-watch-headline mb-3'>{children}</strong>,
    [BLOCKS.PARAGRAPH]: (_, children) => <p className='state-to-watch-text mb-1'>{children}</p>
  }
}
...
function StateWatchlistItem({ state }) {
  return (
    ...
    >
      ...
      <div>{documentToReactComponents(state.watchlistContent.json, documentOptions)}</div>
    </div>
  )
}
