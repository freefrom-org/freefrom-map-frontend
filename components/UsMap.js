    ...
    renderMap() {
        ...
        function zoomNortheast() {
            if (northeastZoom) {
                ...
                document.getElementById('us-map-svg').setAttribute('viewBox', '0 0 ' + width + ' ' + height)
            } else {
        ...
        let mapContainer = d3.select('#us-map').append('div').attr('id', 'map-container').style('max-width', width)
        ...
        d3.select('#map-container')
            ...
            .on('click', function () {
                zoomNortheast(d3.select(this).text())
            })
        ...
        tooltip.attr('id', 'tooltip').append('text')
        ...
        svg.on('mouseover', (d) => {
            ...
            ReactDOM.render(<StateCard hideLearnMore state={d.properties} />, document.getElementById('tooltip'))
        })
            .on('mouseout', () => {
                // Hide tooltip
