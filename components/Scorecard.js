...
const HonorableMention = ({ honorableMentionData }) => (
  <div className="honorable-mention">
    <div className="d-flex flex-row align-items-center">
      <img
        src="/images/honorable-mention.png"
...
const InnovativePolicyIdea = ({ innovativePolicyIdeaData }) => (
  <div className="honorable-mention">
    <div className="d-flex flex-row align-items-center">
      <img
        src="/images/innovative-idea.png"
        className="checklistIcon"
        alt="Innovative Idea"
      />
      <h3 className="m-0" style={{ fontSize: "1em" }}>
    ...
    <p className="card-body small m-0 pt-1">
      ...
      <a
        href={innovativePolicyIdeaData.url}
        target="_blank"
        rel="noopener noreferrer"
      >
...
const renderIcon = (implementsPolicy) => {
  switch (implementsPolicy) {
    ...
    case "maybe":
      return (
        <img
          src="/images/criteria-maybe-met.png"
          className="checklistIcon"
          alt="Criteria Maybe Met"
        />
      );
    case "no":
      return (
        <img
          src="/images/criteria-not-met.png"
          className="checklistIcon"
...
const NoPolicies = ({ adverse }) => (
  <p className="card-body small m-0 d-flex flex-row">
    This state does not have {adverse ? "adverse" : "model"} policies in this
    category.
  </p>
);
...
const Category = ({ category, expanded, onClickExpand, stateData }) => {
  const criteriaWithScores = category.criteria.map((criteria) => {
    const score = stateData.criterion_scores.find(
      (score) => score.criterion_id === criteria.id
    );
    return {
      ...criteria,
      score,
  ...
  const positiveHasPolicies =
    criteriaWithScores.filter((c) => !c.adverse && meetsCriterion(c)) || [];
  const positiveMissingPolicies =
    criteriaWithScores.filter((c) => !c.adverse && !meetsCriterion(c)) || [];
  const adverseHasPolicies =
    criteriaWithScores.filter((c) => c.adverse && meetsCriterion(c)) || [];
  const adverseMissingPolicies =
    criteriaWithScores.filter((c) => c.adverse && !meetsCriterion(c)) || [];
  ...
  const categoryImageName = category.title
    .replace(/\(|\)/g, "")
    .replace(/\s/g, "-")
    .toLowerCase();
  ...
  return (
    <div className="scorecard-container">
      <div className="overall d-flex flex-wrap mt-4 mb-3">
        <span
          className="label mr-2 mb-2 mb-lg-0"
      ...
      <div className="overall d-flex flex-wrap mt-4 mb-3">
        <span className="label mr-2" style={{ textTransform: "capitalize" }}>
