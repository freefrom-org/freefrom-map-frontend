...
async function run() {
  ...
  for (const cat of cats) {
    ...
    for (const crit of cat.criteria) {
      await createOrUpdate("criteria", { id: crit.id, text: crit.title }, [
        "text",
      ]);
    ...
    await createOrUpdate("category", cat, ["title", "help_text"], {
      ideal_criteria,
      adverse_criteria,
    });
  }
  ...
  for (const s of states) {
    ...
    for (const sc of s.category_grades) {
      ...
      for (const critScore of s.criterion_scores) {
        ...
        const inCategory = !!cat.criteria.find(
          (c) => c.id === critScore.criterion_id
        );
        if (inCategory) {
      ...
      for (const link of s.resource_links.filter(
        (r) => r.category_id === sc.category_id
      )) {
      ...
      for (const hm of s.honorable_mentions.filter(
        (r) => r.category_id === sc.category_id
      )) {
      ...
      for (const pi of s.innovative_policy_ideas.filter(
        (r) => r.category_id === sc.category_id
      )) {
      ...
      await createOrUpdate(
        "state_category",
        stateCat,
        Object.keys(stateCat).filter((f) => f !== "id"),
        {
          category,
          criteria_met,
          criteria_maybe_met,
    ...
    await createOrUpdate(
      "state",
      s,
      ["name", "code", "quote", "total", "grade"],
      { categories: stateCategoryRefs }
    );
  }
}
